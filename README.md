# Risk Map of Regional Media Agendas for Central-Bank Communication

Этот репозиторий содержит MVP-пайплайн для построения карты рисков региональной медиаповестки по локальным онлайн-новостям Великобритании. Проект решает задачу раннего выявления тем, которые могут быть значимы для коммуникации центрального банка на региональном уровне.

Пайплайн включает пять шагов:

1. очистка корпуса и нормализация временной и региональной привязки;
2. построение набора статей-кандидатов;
...
Текущие публичные результаты относятся к пилотному режиму (выборка из 120 тыс. уникальных историй). Это означает, что семантический этап выполнялся на выборке story-level наблюдений, а не на полном корпусе.
...
- `Semantic_Map_BoE.pptx` — презентация проекта.
- `RISK_INDEX_GUIDE.md` — гайд по расчёту индекса и устройству пайплайна.
...
- `data/output/public_cleaning_summary.json` — компактная публичная сводка по этапу очистки корпуса.
...
- полные parquet-артефакты и эмбэдинги;
...
Подробное описание всех переменных, формул и интерпретации индекса приведено в [RISK_INDEX_GUIDE.md](RISK_INDEX_GUIDE.md).
...
- `Semantic_Map_BoE_public.pptx` — презентация проекта.
